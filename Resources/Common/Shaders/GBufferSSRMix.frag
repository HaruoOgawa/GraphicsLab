...
layout(binding = 16) uniform sampler2D ssrTexture;
layout(binding = 18) uniform sampler2D mainResultTex;
#else
...
layout(binding = 16) uniform texture2D ssrTexture;
layout(binding = 17) uniform sampler ssrTextureSampler;
layout(binding = 18) uniform texture2D mainResultTex;
layout(binding = 19) uniform sampler mainResultTexSampler;
#endif
...
vec4 GetSSRTexture(vec2 uv)
{
	vec4 col = vec4(0.0);

	#ifdef USE_OPENGL
	col = texture(ssrTexture, uv);
	#else
	col = texture(sampler2D(ssrTexture, ssrTextureSampler), uv);
	#endif
...
GBufferResult GetGBuffer(vec2 ScreenUV)
{
    ...
    gResult.receiveSSR = int(CustomParam0.a);
...
vec4 MixSSR(GBufferResult gResult, vec2 ScreenUV)
{
    vec4 ssr = GetSSRTexture(ScreenUV);
    ssr.r = max(0.0, ssr.r);
    ssr.g = max(0.0, ssr.g);
    ssr.b = max(0.0, ssr.b);
    ssr.a = max(0.0, ssr.a);
...
void main()
{
    ...
    if(gResult.materialType == 1.0 && gResult.receiveSSR == 1)
    {
        // SSRをMix
        col.rgb += MixSSR(gResult, ScreenUV).rgb;
    }
